...
function findUISchemaElement( needle, haystack ) {
	return haystack.find( ( el ) => {
		return el.name === needle;
	} );
}

function assertUISchemaElementDefaults( subschemaName, uischema ) {
	const uiSchemaElement = findUISchemaElement( subschemaName, uischema );
...
describe( 'UISchema', () => {
	...
	it( 'should produce a UI schema given a Json schema', () => {
		const actual = buildUISchema( testJsonSchema, editorConfig );
		...
		for ( const prop in testJsonSchema.properties ) {
			assertUISchemaElementDefaults( prop, actual.elements );
		}
	} );
} );
